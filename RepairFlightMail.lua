...
frame:SetScript("OnEvent",function(self,event,...)
	if (IsAddOnLoaded("pfQuest")) then
		...
		local meta_mail = { ["addon"] = "RFMMM",["texture"] = mail_icon}
		local meta_repair = { ["addon"] = "RFMMM",["texture"] = repair_icon}
		...
		if (UnitFactionGroup("Player") ~= "Horde") then 
			DEFAULT_CHAT_FRAME.editBox:SetText("/db taxi alliance") ChatEdit_SendText(DEFAULT_CHAT_FRAME.editBox, 0)
		...
		if (UnitFactionGroup("Player") ~= "Alliance") then 
			DEFAULT_CHAT_FRAME.editBox:SetText("/db taxi horde") ChatEdit_SendText(DEFAULT_CHAT_FRAME.editBox, 0)
